...
3. **プロダクトバックログ作成**
   - [ ] エピック・ユーザーストーリーの洗い出し
     - [ ] 認証・認可（4ロール管理、メールアドレス管理）
     - [ ] 日報管理（作成、提出、通知、勤務地選択）
     - [ ] 休日登録（登録、Outlook連携、勤務表自動反映）
     - [ ] 勤務表管理（日次入力、月次提出、自動出力、設定管理）
     - [ ] 通知機能（未提出アラート、勤務表入力未完了通知）
     - [ ] インフラ構築（IaC、Lambda別プロジェクト）
   - [ ] ストーリーポイントまたは相対見積りの実施
   - [ ] バックログ管理ツールの初期設定（例: Jira, Linear）

**成果物**: 合意済みの要件定義書、優先順位付きプロダクトバックログ
...
| M3: MVP リリース | Phase 3 完了時 | 日報・勤務表が利用可能な MVP |
| M4: 拡張機能リリース | Phase 4 完了時 | 休日申請・ネタ帳が利用可能 |
...
- 改善したいポイント:
  - 休暇申請・承認をシステム内で一元管理
  - 承認結果を Outlook カレンダーに自動登録し、管理者が確認できるようにする
  - 休暇状況ダッシュボードを整備
